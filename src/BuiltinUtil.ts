...
export async function loadFromDir(dirPath: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    if (file.endsWith('.js')) {
      const imported = await import(fullPath.replace(__dirname, '.')) as { default: { default: BuiltinImplementation }};
      const imp = imported.default.default;
...
export function* generateBuiltinResultClauses(root: RootClause, cache?: BuiltinCache):
IterableIterator<BuiltinBindResult> {
...
export function bindClauseBuiltins(root: RootClause, clause: Clause, cache: BuiltinCache):
  { clause?: Clause; binding: Binding } | undefined {
