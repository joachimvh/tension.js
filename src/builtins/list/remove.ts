...
const check: BuiltinCheckFn = ({ quad }: BuiltinCallOptions): boolean | undefined => {
  ...
  const filtered = left.value.filter(
    (leftEntry): boolean => right.value.some((rightEntry): boolean => fancyEquals(leftEntry, rightEntry)),
  );
...
const bind: BuiltinBindFn = ({ quad }: BuiltinCallOptions): Binding | undefined => {
  ...
  const filtered = left.value.filter((leftEntry): boolean => right.value.some((rightEntry): boolean =>
    fancyEquals(leftEntry, rightEntry)));
